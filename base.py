...
class OCRProvider(ABC):
    ...
    On Windows this uses Windows OCR (UWP) with Tesseract fallback.
    On Linux this uses Tesseract directly.
    """
    ...
    def ocr_region(
        ...
        """Extract text and word bounding boxes from a screen region.

        Args:
            window_title: Capture only this window. If None, full screen.
            region: (x, y, width, height) sub-region to OCR.
            language: Tesseract language code (default: 'eng').

        Returns:
            Dict with: 'success', 'text' (full text), 'words' (list of
    ...
    def list_languages(self) -> list[str]:
        """List available OCR language codes."""


class EscalationProvider(ABC):
    """Find UI elements using escalating strategies.

    Tries fast/cheap methods first (accessibility tree), then OCR,
    then screenshot as last resort.
    """
    ...
    def smart_find(
        ...
        """Find a UI element using escalating search strategies.

        Args:
            name: Element name or visible text to find.
            role: Element role filter (e.g., 'button', 'text').
            window_title: Limit search to this window.

        Returns:
            Dict with: 'success', 'method' ('atspi'|'ocr'|'screenshot'),
            'element' (dict with name/role/bounds), 'confidence' (float).
        """
...
class CascadeRecoveryProvider(ABC):
    ...
    def cascade_find(
        ...
        Args:
            name: Element name or text to find.
            role: Element role filter.
            window_title: Limit search to this window.
            strategies: Override default strategy order.

        Returns:
            Dict with: 'success', 'method', 'strategy_used',
            'element', 'attempts' (list of tried strategies).
        """


class SoMProvider(ABC):
    """Set-of-Mark annotation — numbered labels on UI elements.

    Overlays numbered [1], [2], [3] labels on interactive UI elements
    in a screenshot so the LLM can refer to elements by number.
    """
    ...
    def get_annotated_screenshot(
        ...
        """Annotate a screenshot with numbered labels on interactive elements.

        Args:
            window_title: Target window. If None, uses focused window.
            max_depth: Max tree depth for element discovery.

        Returns:
            Dict with: 'success', 'image' (base64 PNG), 'elements' (list of
            {'index', 'name', 'role', 'bounds', 'actions', 'path'}),
            'element_count', 'window_title'.
        """

    @abstractmethod
    def som_click(self, index: int, action: str = "click") -> dict:
        """Click an element by its SoM index from the last annotation.

        Args:
            index: The numbered label from get_annotated_screenshot.
            action: AT-SPI2 action name (default: 'click').

        Returns:
            Dict with: 'success', 'method' ('atspi_action'|'coordinate_click'),
            'element' (dict with name/role/bounds).
        """
