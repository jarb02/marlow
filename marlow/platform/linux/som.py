...
# Actions that don't indicate a truly interactive element
_IGNORE_ACTIONS = {"clickAncestor", "scrollTo"}

# Container roles — only collect if they're in _INTERACTIVE_ROLES
_CONTAINER_ROLES = {
    "frame", "window", "panel", "filler", "section", "group",
    "tool bar", "menu bar", "status bar", "scroll pane",
    "viewport", "internal frame", "document", "application",
    "alert", "separator", "menu",
}
...
class LinuxSoMProvider(SoMProvider):
    ...
    def get_annotated_screenshot(
        ...
        # On Wayland/Sway, AT-SPI2 typically returns window-relative
        # coordinates (not screen-absolute), so no offset adjustment needed.
        win_offset_x = 0
        win_offset_y = 0
    ...
    def _collect_interactive(self, node: dict, results: list):
        ...
        real_actions = [a for a in actions if a not in _IGNORE_ACTIONS]
        is_interactive = role in _INTERACTIVE_ROLES
        # Non-container roles with real actions are also interactive
        if not is_interactive and real_actions and role not in _CONTAINER_ROLES:
            is_interactive = True
